                                   ...
                                   implements IViewportController
{
    ...
    private static const DEFAULT_MOUSE_WHEEL_ZOOM_FACTOR:Number = 1.11
		...
		private static const DRAG_THRESHOLD_DISTANCE:Number = 200 // pixels
    ...
    private var mouseDownPosition:Point
		...
		public var stopPosition:Point
		public var resPosition:String
		
		public var slope:Number
		private var _direction:String		
		
		public function get directionDrag():String
		{
			return _direction;
		}
		
		public function set directionDrag(value:String):void
		{
			_direction = value;
		}
    ...
    override protected function view_removedFromStageHandler(event:Event):void
    {
		 ...
		 * @private
		 */
		public function view_startDragHandler(event:MouseEvent):void
		{
			// prevent flipping when panning
			/*if (panning)
				return			*/
				
			view.addEventListener(MouseEvent.MOUSE_UP,
				view_stopDragHandler,
				false, 0, true)
										
			// remember mouse position
			startPosition = new Point(view.mouseX, view.mouseY)
		 ...
		 * @private
		 */
		public function view_stopDragHandler(event:MouseEvent):void
		{
			// prevent flipping when panning
			/*if (panning)
				return*/			
			
			view.removeEventListener(MouseEvent.MOUSE_UP,
				view_stopDragHandler)
/*			view.removeEventListener(MouseEvent.ROLL_OUT,
				view_rollOutHandler)
			...
			stopPosition = new Point(view.mouseX, view.mouseY)
			var px:Number = stopPosition.x - startPosition.x
			var py:Number = stopPosition.y - startPosition.y
			
			resPosition = "(x="+px+", "+"y="+py+")"
			var m:Number
			
			if(py > 0 && Math.abs(py) > DRAG_THRESHOLD_DISTANCE){
				stopPanning();
				_direction = DIRECTION_DOWN;
			}else if(py < 0 && Math.abs(py) > DRAG_THRESHOLD_DISTANCE){
				stopPanning();
